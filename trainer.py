...
class SPINTrainer(Trainer):
    ...
    def __init__(
        ...
        if peft_config is not None:
            ...
            if getattr(model, "is_loaded_in_8bit", False) or getattr(model, "is_loaded_in_4bit", False):
                ...
                # NEW: enable gradient checkpointing and disable cache
        ...
        # NEW: freeze reference model parameters (no gradients)
        ...
        if self.ref_model is None:
            ...
            # NEW: ensure ref_model is still frozen after wrapping
            for p in self.ref_model.parameters():
                p.requires_grad = False
        ...
        #         self.ref_model.resize_token_embeddings(len(tokenizer))
    ...
    def spin_loss(
        ...
        """Compute the SPIN loss with optional KL regularization."""
        ...
        # rewards
        real_rewards = self.beta * (policy_real_logps - opponent_real_logps)
        generated_rewards = self.beta * (policy_generated_logps - opponent_generated_logps)
        logits_real = policy_real_logps - opponent_real_logps
        logits_generated = policy_generated_logps - opponent_generated_logps

        reward_max = 0.5 * torch.ones_like(real_rewards) # Qwen3-4B
        reward_min = -0.5 * torch.ones_like(generated_rewards)
        # reward_max = 1.0 * torch.ones_like(real_rewards) # Mistral-7B
        # reward_min = -1.0 * torch.ones_like(generated_rewards)

        base_losses = 0.5 * (self.beta * logits_real - reward_max) ** 2 + 0.5 * (self.beta * logits_generated - reward_min) ** 2

        # KL regularization (per-example, then averaged)
        # Approximate KL as log-ratio differences
        kl_real = policy_real_logps - opponent_real_logps
        kl_generated = policy_generated_logps - opponent_generated_logps
        kl_per_example = 0.5 * (kl_real + kl_generated)

        # Add penalty
        losses = base_losses + 0.005 * ((self.beta * logits_real) ** 2 + (self.beta * logits_generated) ** 2)

        print(f"losses: {losses}")
        print(f"kl_per_example: {kl_per_example}")
        print(f"kl mean: {kl_per_example.mean().item()}")
    ...
    def get_batch_metrics(
        ...
        (
            ...
            policy_real_logits_mean,
            policy_generated_logits_mean,
        ...
        # these are now per-example means → scalar-friendly
        metrics[f"{prefix}logits/generated"] = policy_generated_logits_mean.detach().cpu().mean()
        metrics[f"{prefix}logits/real"] = policy_real_logits_mean.detach().cpu().mean()
    ...
    # def get_batch_samples(self, model, batch: Dict[str, torch.LongTensor], device=None) -> Tuple[str, str]:
    ...
    def generate_samples_for_logging(self, batch: Dict[str, torch.Tensor]):
        """
        Helper used only for W&B game_log in evaluation_loop:
        generate policy and reference completions for a *single* already-collated batch.
        """
        ...
        if tokenizer is None:
            raise ValueError("Cannot sample without a tokenizer or processing_class.")
        ...
        max_new_tokens = getattr(self.args, "generation_max_new_tokens", None)
        if max_new_tokens is None:
            if self.is_encoder_decoder and self.max_target_length is not None:
                max_new_tokens = self.max_target_length
            elif (self.max_length is not None) and (self.max_prompt_length is not None):
                max_new_tokens = self.max_length - self.max_prompt_length

        if (
            (max_new_tokens is None or max_new_tokens <= 0)
            and self.max_length is not None
            and "prompt_attention_mask" in batch
        ):
            prompt_lengths = batch["prompt_attention_mask"].sum(dim=1).max().item()
            remaining = self.max_length - int(prompt_lengths)
        ...
        policy_output = self.model.generate(**generation_kwargs)

        # if self.ref_model is None:
        #     # disable adapters on main model for reference generation
        #     with self.accelerator.unwrap_model(self.model).disable_adapter():
        #         reference_output = self.model.generate(**generation_kwargs)
        # else:
        #     reference_output = self.ref_model.generate(**generation_kwargs)

        pad_length = self.max_length if self.max_length is not None else policy_output.shape[1]
        policy_output = pad_to_length(policy_output, pad_length, pad_token_id)

        # ref_pad_length = self.max_length if self.max_length is not None else reference_output.shape[1]
        # reference_output = pad_to_length(reference_output, ref_pad_length, pad_token_id)
    ...
    def get_batch_samples(self, epoch_iterator, num_batches: int, device=None):
        ...
        HF Trainer API (>=4.46):
            ...
            get_batch_samples(epoch_iterator, num_batches, device)
                -> (batch_samples, num_items_in_batch)

        `epoch_iterator` is an iterator over the train dataloader created by
        `Trainer.get_train_dataloader`, so it already uses `self.data_collator`.

        Here we simply draw up to `num_batches` micro-batches from the iterator,
        move them to the right device using `_prepare_inputs`, and return them.
        `compute_loss` will then be called on each element in `batch_samples`.
        """
        ...
        for _ in range(num_batches):
            try:
                ...
                # End of dataloader for this epoch
                break

            # Move batch to device etc. (also applies data type casting)
            batch = self._prepare_inputs(batch)

            batch_samples.append(batch)

            # For now, just count number of examples for normalization.
            # You can switch to token-level counts if you want later.
            if isinstance(batch, dict):
                first_key = next(iter(batch))
                if isinstance(batch[first_key], torch.Tensor):
                    bsz = batch[first_key].size(0)
                    total_items += bsz

        num_items_in_batch = total_items if total_items > 0 else None
    ...
    def evaluation_loop(
        ...
        if self.generate_during_eval:
            ...
            # generate samples for logging (this handles _prepare_inputs internally)
            policy_output_decoded, ref_output_decoded = self.generate_samples_for_logging(random_batch)
