...
def sequence_logprob_from_logits(
    ...
    returns: sum log p over non-masked tokens (or mean if length_normalize=True), shape [B]
    """
    logits = logits[:, :-1, :]
    labels = labels[:, 1:]

    loss_mask_bool = labels != -100
    safe_labels = labels.masked_fill(~loss_mask_bool, 0)
    ...
    loss_mask = loss_mask_bool.to(per_token_logps.dtype)
...
def precompute_ref_logps_for_dataset(
    ...
    for start in range(0, n, batch_size):
        ...
        logp_ref_pos = sequence_logprob_from_logits(
            pos_out.logits,
            pos_labels,
        ...
        logp_ref_neg = sequence_logprob_from_logits(
            neg_out.logits,
            neg_labels,
            length_normalize=length_normalize,
        )
...
class PairwiseRatioMSETrainer(SFTTrainer):
    ...
    def _seq_logprob(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        return sequence_logprob_from_logits(logits, labels, length_normalize=self.length_normalize)
...
def main():
    ...
    ap.add_argument("--length_normalize", action="store_true",
                    help="If set, average sequence logprob over loss-masked tokens instead of summing.")
    ap.add_argument(
