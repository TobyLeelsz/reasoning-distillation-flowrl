...
def process_item(config, data_source, response_lst, reward_data):
    ...
    for r in response_lst:
        try:
            ...
            print(f"[Warning] Failed to score response: {e}")  # [Modified]
    return data_source, np.mean(score_lst) if score_lst else 0.0  # [Modified]
...
def main(config):
    ...
    with tqdm(total=total) as pbar:
        while len(remote_tasks) > 0:
            ...
            for result_id in done_ids:
                ...
                try:
                    data_source, score = ray.get(result_id)  # [Modified]
                    data_source_reward[data_source].append(score)
                except Exception as e:
                    print(f"[Warning] Worker crashed on one task: {e}")  # [Modified]
                pbar.update(1)
    ...
    json_output_path = os.path.join(output_dir, "results_eval.json")
