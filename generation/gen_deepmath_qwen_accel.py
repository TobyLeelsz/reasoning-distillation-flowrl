...
_HF_LIST_COMPAT_PATCHED = False


def ensure_hf_list_feature_compat() -> bool:
    """
    Compatibility shim for caches/datasets created by newer HF Datasets versions
    that serialize variable-length lists with `_type: "List"`.
    In datasets==3.5.0, list<struct<...>> is represented as `[{"...": Feature}]`,
    not Sequence/LargeList; patch generate_from_dict accordingly.
    """
    global _HF_LIST_COMPAT_PATCHED

    if "List" in hf_features._FEATURE_TYPES or _HF_LIST_COMPAT_PATCHED:
        return False

    def _generate_from_dict_with_list(obj: Any):
        # Keep original behavior for nested structures.
        if isinstance(obj, list):
            return [_generate_from_dict_with_list(value) for value in obj]
        if "_type" not in obj or isinstance(obj["_type"], dict):
            return {key: _generate_from_dict_with_list(value) for key, value in obj.items()}

        obj = dict(obj)
        _type = obj.pop("_type")

        if _type == "List":
            feature = _generate_from_dict_with_list(obj.pop("feature"))
            if isinstance(feature, (dict, tuple, list)):
                return [feature]
            return hf_features.Sequence(feature=feature, **obj)

        class_type = hf_features._FEATURE_TYPES.get(_type, None) or getattr(hf_features, _type, None)
        if class_type is None:
            raise ValueError(
                f"Feature type '{_type}' not found. "
                f"Available feature types: {list(hf_features._FEATURE_TYPES.keys())}"
            )

        if class_type == hf_features.LargeList:
            feature = obj.pop("feature")
            return hf_features.LargeList(feature=_generate_from_dict_with_list(feature), **obj)
        if class_type == hf_features.Sequence:
            feature = obj.pop("feature")
            return hf_features.Sequence(feature=_generate_from_dict_with_list(feature), **obj)

        field_names = {f.name for f in dataclass_fields(class_type)}
        return class_type(**{k: v for k, v in obj.items() if k in field_names})

    hf_features.generate_from_dict = _generate_from_dict_with_list
    _HF_LIST_COMPAT_PATCHED = True
    return True


def load_dataset_with_compat(path: str, split: str):
    try:
        return load_dataset(path, split=split)
    except ValueError as e:
        if "Feature type 'List' not found" not in str(e):
            raise
        patched = ensure_hf_list_feature_compat()
        if not patched:
            raise
        print(
            "[datasets] Applied compatibility shim for feature type 'List'; retrying load_dataset",
            flush=True,
        )
        return load_dataset(path, split=split)
...
def validate_model_for_generation(model, rank: int) -> None:
    ...
    if ndim != 2:
        raise RuntimeError(
            f"[rank{rank}] Invalid input embedding weight shape: {shape}. "
            "This usually means the script was launched with a DeepSpeed ZeRO config "
            "(e.g., zero3_init) that is incompatible with this inference path. "
            "Launch with a non-DeepSpeed Accelerate config, e.g. "
            "`accelerate launch --config_file generation/accelerate_generate.yaml ...`."
        )
...
def main():
    ...
    if args.complement_only:
        ...
        ds = load_dataset_with_compat(args.target_dataset, split=args.target_split)
        ...
        ds = load_dataset_with_compat(args.dataset, split=args.split)
    ...
    model.eval()
    validate_model_for_generation(model, rank)
