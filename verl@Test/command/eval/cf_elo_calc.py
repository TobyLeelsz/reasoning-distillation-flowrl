...
def format_grouped_contest_data(submissions: List[List[bool]], problem_ids: List[str]) -> List[Tuple[int, Dict[str, List[bool]]]]:
    ...
    if isinstance(submissions, pd.DataFrame):
        submissions = submissions["responses"].tolist()
...
def main():
    ...
    try:
        ...
        output_path = os.path.join(os.path.dirname(results_path), "results_codeforces.txt")
        with open(output_path, "w") as f:
            f.write("="*50 + "\n")
            f.write("CODEFORCES PERFORMANCE SUMMARY\n")
            f.write("="*50 + "\n")
            f.write(f"Estimated Percentile: {est_percentile:.1f}%\n")
            f.write(f"Estimated Codeforces Rating: {estimated_rating}\n")
            f.write(f"Contests Processed: {len(contest_elos)}\n")
            f.write("="*50 + "\n")
